...
STUDENT_TEST("Test case that accesses memory after it was deleted") {
    DataPoint* taskList = new DataPoint[8];
    for(int i = 0; i< 8; ++i){
        taskList[i].label = "sleep"+integerToString(i);
    }
    delete[] taskList;

    taskList = new DataPoint[3];
    for(int i = 0; i< 3; ++i){
        taskList[i].label = "wake up"+integerToString(i);
    }
    delete[] taskList;
    ...
    /* taskList points to memory address that is now deleted, so
           executing the line below accesses deleted memory */
    EXPECT_EQUAL(taskList[0].label, "sleep0"); // should label still be sleep? why or why not?
}
